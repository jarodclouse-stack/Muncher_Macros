  ...
  return ReactDOM.createPortal(
    <div style={{ position: 'fixed', inset: 0, background: 'var(--theme-bg, #001114)', zIndex: 99999, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', position: 'relative' }}>
        ...
        {/* TAB CONTENT (Hidden visually when overlay is open to keep focus, but preserved in DOM) */}
        <div style={{ display: configuringFood ? 'none' : 'flex', flexDirection: 'column', flex: 1 }}>
          {activeTab === 'pantry' && (
        ...
        {configuringFood && (
          <div style={{ 
            ...
            background: 'var(--theme-bg, #001114)', 
            ...
            <button onClick={() => setConfiguringFood(null)} style={{ background: 'none', border: 'none', color: 'var(--theme-accent, #00C9FF)', display: 'flex', alignItems: 'center', gap: '4px', cursor: 'pointer', padding: '8px 0', marginBottom: '16px', fontWeight: '600' }}>
              <ChevronLeft size={16} /> Close Preview
            </button>
            
            <div style={{ background: 'var(--theme-panel, rgba(255,255,255,0.05))', borderRadius: '16px', padding: '20px', border: '1px solid var(--theme-border, rgba(255,255,255,0.1))', boxShadow: '0 10px 30px rgba(0,0,0,0.5)' }}>
              <div style={{ marginBottom: '16px' }}>
                <input 
                  value={editName}
                  onChange={e => setEditName(e.target.value)}
                  placeholder="Rename this food..."
                  style={{ width: '100%', boxSizing: 'border-box', background: 'var(--theme-input-bg, rgba(0,0,0,0.3))', border: '1px solid var(--theme-border, rgba(255,255,255,0.2))', padding: '12px', color: 'var(--theme-text)', borderRadius: '12px', fontSize: '16px', fontWeight: '700' }}
                />
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px', marginBottom: '16px' }}>
                <QuickMacro label="CAL" val={Math.round((Number(configuringFood.cal) || 0) * computeMultiplier(configuringFood.serving||'', servingUint, parseFloat(servingQty)||1))} color="#fff" />
                <QuickMacro label="P" val={Math.round((Number(configuringFood.p) || 0) * computeMultiplier(configuringFood.serving||'', servingUint, parseFloat(servingQty)||1))} color="#ff6b6b" />
                <QuickMacro label="C" val={Math.round((Number(configuringFood.c) || 0) * computeMultiplier(configuringFood.serving||'', servingUint, parseFloat(servingQty)||1))} color="#4dabf7" />
                <QuickMacro label="F" val={Math.round((Number(configuringFood.f) || 0) * computeMultiplier(configuringFood.serving||'', servingUint, parseFloat(servingQty)||1))} color="#fab005" />
              </div>

              <form id="serving-form" onSubmit={handleConfirmAdd} style={{ display: 'flex', gap: '12px', marginBottom: '20px' }}>
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', fontSize: '12px', color: 'var(--theme-text-dim, #8b8b9b)', marginBottom: '6px' }}>Quantity</label>
                  <input type="number" step="0.1" required value={servingQty} onChange={e => setServingQty(e.target.value)} style={{ width: '100%', boxSizing: 'border-box', background: 'var(--theme-input-bg, rgba(0,0,0,0.3))', border: '1px solid var(--theme-border, rgba(255,255,255,0.2))', padding: '12px', color: 'var(--theme-text)', borderRadius: '12px', fontSize: '15px', fontWeight: '700' }} />
                </div>
                <div style={{ flex: 1.5 }}>
                  <label style={{ display: 'block', fontSize: '12px', color: 'var(--theme-text-dim, #8b8b9b)', marginBottom: '6px' }}>Unit</label>
                  <select value={servingUint} onChange={e => setServingUnit(e.target.value)} style={{ width: '100%', boxSizing: 'border-box', background: 'var(--theme-input-bg, rgba(0,0,0,0.3))', border: '1px solid var(--theme-border, rgba(255,255,255,0.2))', padding: '12px', color: 'var(--theme-text)', borderRadius: '12px', fontWeight: '600' }}>
                    {COMMON_UNITS.map((u: any) => <option key={u.id} value={u.id} style={{ background: '#101420', color: '#fff' }}>{u.label}</option>)}
                  </select>
                </div>
              </form>
              ...
              <div 
                ...
                style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px', background: saveToPantry ? 'var(--theme-success-dim, rgba(146,254,157,0.1))' : 'rgba(255,255,255,0.02)', border: `1px solid ${saveToPantry ? 'var(--theme-success, #92FE9D)' : 'var(--theme-border, rgba(255,255,255,0.05))'}`, borderRadius: '12px', cursor: 'pointer', transition: 'all 0.2s', marginBottom: '16px' }}>
                ...
                <div style={{ fontSize: '13px', fontWeight: '700', color: saveToPantry ? 'var(--theme-success, #92FE9D)' : 'var(--theme-text)' }}>Save to Pantry</div>
              </div>
              ...
              <div style={{ marginBottom: '12px' }}>
                <button 
                  ...
                  style={{ 
                    ...
                    padding: '10px 14px', background: 'var(--theme-panel, rgba(255,255,255,0.05))',
                    border: '1px solid var(--theme-border, rgba(255,255,255,0.1))', borderRadius: '10px',
                    color: 'var(--theme-text)', cursor: 'pointer'
                  }}
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', fontWeight: '600' }}>
                    <Beaker size={14} color="var(--theme-accent, #00C9FF)" /> Details
                  </div>
                  {showFullNutrition ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                </button>

                {showFullNutrition && (
                  <div style={{ 
                    marginTop: '8px', padding: '12px', background: 'var(--theme-panel, rgba(255,255,255,0.03))',
                    borderRadius: '10px', border: '1px solid var(--theme-border, rgba(255,255,255,0.05))',
            ...
            <button form="serving-form" type="submit" style={{ marginTop: 'auto', padding: '16px', background: 'var(--theme-accent, #00C9FF)', color: 'var(--theme-panel-base, #000)', borderRadius: '14px', border: 'none', fontWeight: '800', fontSize: '16px', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '10px', boxShadow: '0 8px 16px rgba(0,201,255,0.3)' }}>
...
const QuickMacro = ({ label, val, color }: any) => (
  <div style={{ background: 'rgba(255,255,255,0.02)', padding: '10px 4px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)', textAlign: 'center' }}>
    <div style={{ fontSize: '10px', color: 'var(--theme-text-dim)', fontWeight: '800', marginBottom: '2px' }}>{label}</div>
